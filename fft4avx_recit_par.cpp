...
#define CHUNK 256 // number of consecutive vectors
#define BLOCK 65536
...
struct fft_plan {
		...
		while ( st_ptr ) { // stack not empty
			...
			if ( st_next() ) {
                ...
                if ( TOP(SIZE) == BLOCK ) {
                    ...
                    const size_t size = TOP(SIZE);
                    double * subdata = TOP_(DATA);
					double * subtwiddle = TOP_(TWID);

                    for ( size_t step = size; step > 4; step >>= 2 ) {

                          const size_t N4 = step >> 2;

                          #pragma omp parallel for collapse(2) shared(subdata, subtwiddle) schedule(static, CHUNK)
                          for ( size_t i = 0; i < size; i += step ) {
                              for ( size_t n = 0; n < N4; n += vector_width ) {

                                  butterfly( subdata + (n + i) * 2, subtwiddle + n * 6, step );
                              }
                          }
                          subtwiddle += 6 * step >> 2;
                    }

                    #pragma omp parallel for shared(subdata) schedule(static, CHUNK)
                    for ( size_t n = 0; n < size; n += 16 ) {

                        butterfly_final( subdata + n * 2 );
                    }
                    ...
                    #pragma omp parallel for shared(subdata, subtwiddle) schedule(static, CHUNK)
					for ( size_t n = 0; n < N4; n += vector_width ) {

						butterfly( subdata + n * 2, subtwiddle + n * 6, TOP(SIZE) );
					}
					TOP_(DATA) += 2 * TOP(SIZE);

					st_push( N4, subdata, TOP_(TWID) + 6 * N4 );
				}
    ...
    void fft( const double * in, double * out ) {
		...
		//omp_set_dynamic(0);     // Manual thread distribution
		//omp_set_nested(1);      // True for nested parallelism

        // depth 0 with vec reorder re-im-re-im-re-im-re-im   to   re-re-re-re-im-im-im-im
        {
            const size_t N4                           = size >> 2;
            #pragma omp parallel for shared(data, twiddle_data) schedule(static, CHUNK)
            for ( size_t n = 0; n < N4; n += vector_width ) {

                butterfly_zero( in + n * 2, data + n * 2, twiddle_data + n * 6, size );
            }
            twiddle_data += 6 * N4; // 2 * 3 * N4
        }

		if ( size > BLOCK ) fft_recursive( data, twiddle_data ); // unroll
			...
			const size_t top = depth - 1;
                ...
                for ( size_t k = 1; k < top; ++k, step >>= 2 ) {
                      ...
                      #pragma omp parallel for collapse(2) shared(data, twiddle_data) schedule(static, CHUNK)
                      for ( size_t i = 0; i < size; i += step ) {
                          for ( size_t n = 0; n < N4; n += vector_width ) {

                              butterfly( data + (n + i) * 2, twiddle_data + n * 6, step );
                          }
                      }
                      twiddle_data += 6 * step >> 2;
                }

            #pragma omp parallel for shared(data) schedule(static, CHUNK)
		    for ( size_t n = 0; n < size; n += 16 ) {

		        butterfly_final( data + n * 2 );
		    }
