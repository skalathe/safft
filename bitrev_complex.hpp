...
/*************************************************************************************
    ...
    Bit Reversal Algorithm based on COBRA Shuffle

    + Fast (Twice as fast as original inplace COBRA Shuffle)
    + Cache effective
    ...
    - Limited LOG_B * 2 <= LOG_N <= LOG_B * 4 (LOG_B = log_2 side of COBRA table, LOG_N = log_2 of problem size N)
*************************************************************************************/
...
/*
    ...
    Table memory can be specified but needs size (get with size()) or can be obtained with allocate() function
*/
...
class BitRev {
  public:
    ...
    inline static unsigned_t rev_r( const unsigned_t & r, std::enable_if_t<0 < LBR, bool> = true ) {

        const unsigned_t a = r & LOG_B_BIT;
        const unsigned_t b = (r - a) >> (unsigned_t) LOG_BR;
        return (brev::data[a] << (unsigned_t) LOG_BR) + brev::data[b];
    }
    ...
    template <signed_t LBR>
    inline static unsigned_t rev_r( const unsigned_t & r, std::enable_if_t<LBR < 0, bool> = true ) {

        return brev::data[r] >> (unsigned_t) -LOG_BR;
    }

    inline static void table_reverse( const unsigned_t & r, const unsigned_t & r_, T * __restrict cache_block, T * __restrict const arr ) {

        const unsigned_t spec = r < r_ ? 0 : 1;
        T * __restrict array_block  = arr + r  * B * 2;
        T * __restrict array_block_ = arr + r_ * B * 2;
    ...
    inline static void reverse( T * __restrict const arr, T * __restrict cache_block ) {
        ...
        /*/
        // Alternative loop - every iteration have work to do
        constexpr static unsigned_t LOG_R = (LOG_N - 2 * LOG_B + 1) >> 1;
        constexpr static unsigned_t SQRT_R = 1 << LOG_R;

        for ( unsigned_t k = 0, block = 0; k < SQRT_R; ++k ) {
            for ( unsigned_t l = block++; l < SQRT_R; ++l ) {

                const unsigned_t m  = k * SQRT_R + l;
                const unsigned_t m_ = rev_r<LOG_BR>(m);
                const unsigned_t r  = (m_ >> LOG_R) + k * SQRT_R;
                const unsigned_t r_ = rev_r<LOG_BR>(r);

                if ( r == r_ )
                    table_reverse( r, r_, cache_block, arr );
                else {
                    table_reverse( r, r_, cache_block, arr );
                    table_reverse( r_, r, cache_block, arr );
                }
            }
        }
        /*/
        // Basic loop - do half iterations skipped but with less computing (sequentially is faster)
        for ( unsigned_t r = 0; r < R; ++r ) {
